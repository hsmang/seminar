    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/board/multiplePhotoUpload.do'; 	
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
    	var oAjax = jindo.$Ajax(sUploadURL, {
			type: 'xhr',
			method : "post",
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수ㄹ
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			...
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/seminar/insert.do',
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/resources/se2/sample/photo_uploader/callback.html',	
